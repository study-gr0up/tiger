...
public class Main
{
	...
	if (exp instanceof Exp.Id) {
	    ...
	    Exp.Op e = (Exp.Op) exp;
	    // Also in an opExp there still may have embedded print.(recursive invoke print)
	...
	if (explist instanceof ExpList.Pair) {
	    ...
	    // For Current Explist, the args num is below
	    int n1 = 1 + maxArgsExpList(el.list);
	    // May be a EsepExp in the ExpList, So may be a embedded print in it.(recursive invoke print)
	    int n2 = maxArgsExp(el.exp);
	    // Also maybe a print in ExpList
	    int n3 = maxArgsExpList(el.list);
	    int temp = n1 >= n2 ? n1 : n2;
	    return n3 >= temp ? n3 : temp;
	} else if (explist instanceof ExpList.Last) {
	    ExpList.Last el = (ExpList.Last) explist;
	    int n1 = maxArgsExp(el.exp);
	    return n1 >= 1 ? n1 : 1;
	} else
	    new Bug();
	return 0;
